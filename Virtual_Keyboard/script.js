        ...
        switch (key) {
            ...
            case 'Enter':
                if (virtualKeyboardChromeExtensionClickedElem != null) {
                    ...
                    virtualKeyboardChromeExtensionClickedElem.dispatchEvent(enterEvent);

                    const inputEvent = new Event('input', { bubbles: true });
                    virtualKeyboardChromeExtensionClickedElem.dispatchEvent(inputEvent);

                    const changeEvent = new Event('change', { bubbles: true });
                    virtualKeyboardChromeExtensionClickedElem.dispatchEvent(changeEvent);
                }
                break;

            case 'Backspace':
                if (virtualKeyboardChromeExtensionClickedElem != null) {
                    const pos = virtualKeyboardChromeExtensionClickedElem.selectionStart;
                    const posEnd = virtualKeyboardChromeExtensionClickedElem.selectionEnd;

                    if (pos > 0) {
                        virtualKeyboardChromeExtensionClickedElem.value = 
                            virtualKeyboardChromeExtensionClickedElem.value.slice(0, pos - 1) +
                            virtualKeyboardChromeExtensionClickedElem.value.slice(posEnd);
                        virtualKeyboardChromeExtensionClickedElem.selectionStart = pos - 1;
                        virtualKeyboardChromeExtensionClickedElem.selectionEnd = pos - 1;
                        ...
                        const backspaceEvent = new KeyboardEvent('keydown', { key: 'Backspace', keyCode: 8, bubbles: true });
                        virtualKeyboardChromeExtensionClickedElem.dispatchEvent(backspaceEvent);
                    }
                }
                break;
            ...
            default:
                if (virtualKeyboardChromeExtensionClickedElem != undefined) {
                    ...
                    // Shift basılıysa büyük/küçük harf dönüşümü yap
                    if (virtualKeyboardChromeExtensionShift) {
                        if (key.length === 1) { // Sadece tek karakterli tuşlar için uygula
                            if ((key >= 'a' && key <= 'z') || (key >= 'A' && key <= 'Z')) {
                                key = key.toUpperCase();
                            }
                        }

                        if (virtualKeyboardChromeExtensionShiftBehaviour) {
                            virtualKeyboardChromeExtensionShift = false;
                            document.getElementById("virtualKeyboardChromeExtensionMainKbd").className = "";
                            virtualKeyboardChromeExtension_shiftButtonKeys();
                        }
                    }
                    ...
                    if (pos >= 0) {
                        ...
                        virtualKeyboardChromeExtensionClickedElem.dispatchEvent(keydownEvent);
                    }
                }
                break;
        }
...
function vk_ajax_load_main() {
  if (xhr.readyState == 4) {
	...
	chrome.extension.sendRequest({method: "initLoadKeyboardSettings"}, function(response) {
		...
		// zoomLevel
		if (response.zoomLevel == undefined) { response.zoomLevel = 0.8; }
